...
class AIAssistant:
    ...
    def get_academic_context(self, user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if user_data.get('purpose') == 'school' and user_data.get('school'):
            school = user_data['school']
            context["grade"] = school.get('grade')
            context["board"] = school.get('board')
            context["stream"] = school.get('stream')
            context["subjects"] = school.get('subjects', [])
            # Get subjects from syllabus
            from templates.academic_data import get_syllabus
            syllabus = get_syllabus('school', school.get('board'), school.get('grade'), subject_combination=school.get('subject_combination'))
            context["subjects"] = list(syllabus.keys()) if syllabus else []

        elif user_data.get('purpose') == 'exam' and user_data.get('exam'):
        ...
        # Get progress data
        from sclera import calculate_academic_progress
    ...
    def save_message(self, uid: str, chatbot_type: str, role: str, content: str):
        ...
        try:
            ...
            from sclera import get_user_data
